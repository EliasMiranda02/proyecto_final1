document.getElementById('searchFormReservasv').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    fetch('controlador/buscar_reservav.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
    .then(data => {
        const tbody = document.querySelector('#Reservav table tbody'); // Especifica el tbody correcto
...
btnEditar.addEventListener('click', function(event) {
    ...
    if (checkedCheckboxes.length === 1) {
        ...
        document.getElementById('id_reservav_editar').value = id;
