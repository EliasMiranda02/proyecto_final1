...
async function loadPackages() {
    try {
        const response = await fetch('controlador/rellenar_form_reservaea.php');
        const data = await response.json();

        const packageSelect = document.getElementById("excursion");

        // Iterar sobre los datos y añadir opciones al combobox
        data.forEach(package => {
            packageData[package.id_excursion] = package;
            const option = document.createElement("option");
            option.value = package.id_excursion; // Asigna el ID del paquete como value
            option.textContent = package.ubicacion; // Muestra el nombre del paquete como texto
...
function fillPackageData() {
    const packageId = document.getElementById("excursion").value;
    ...
    if (packageId && packageData[packageId]) {
        document.getElementById("precio").value = packageData[packageId].precio; // Muestra el precio del paquete
    } else {
...
async function loadPackages1() {
    try {
        const response = await fetch('controlador/rellenar_form_reservaeaciudad.php');
        const data = await response.json();
        ...
        // Iterar sobre los datos y añadir opciones al combobox
        data.forEach(package => {
            packageData1[package.id_salidab] = package;
            const option = document.createElement("option");
            option.value = package.id_salidab; // Asigna el ID del paquete como value
