document.getElementById('searchFormReservapv').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    fetch('controlador/buscar_reservapv.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
    .then(data => {
        const tbody = document.querySelector('#Reservapv table tbody'); // Especifica el tbody correcto
...
btnEditar.addEventListener('click', function(event) {
    ...
    if (checkedCheckboxes.length === 1) {
        ...
        document.getElementById('id_reservapv_editar').value = id;
