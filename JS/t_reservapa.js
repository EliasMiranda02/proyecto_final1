document.getElementById('searchFormReservaspa').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    fetch('controlador/buscar_reservaspa.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
    .then(data => {
        const tbody = document.querySelector('#Reservapa table tbody'); // Especifica el tbody correcto
...
btnEditar.addEventListener('click', function(event) {
    ...
    if (checkedCheckboxes.length === 1) {
        ...
        document.getElementById('id_reservapa_editar').value = id;
