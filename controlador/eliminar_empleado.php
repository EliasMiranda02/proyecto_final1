...
if (isset($_POST['ids']) && !empty($_POST['ids'])) {
    ...
    if (!empty($validIds)) {
        ...
        $sql = "DELETE FROM empleados WHERE id_empleado IN ('$idsString')"; // Usa IN para eliminar múltiples registros

        if ($conexion->query($sql) === TRUE) {
            // Redirigir con éxito
            header('Location: ../index.php?i=empleado&mensaje=eliminado');
            exit();
        } else {
            // Manejar errores
        ...
        header('Location: ../index.php?i=empleado&mensaje=id_invalido');
        exit();
    }
} else {
